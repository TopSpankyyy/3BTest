...
To add or rename a system, update the `SYSTEMS` list in [Submit new hire](<Submit new hire/script.ts>), [List new hires](<List new hires/script.ts>), [Update system status](<Update system status/script.ts>), and [the form](<New hire form/App.tsx>) — new systems need a matching column added to the table.
