...
Field labels differ from column names on purpose: "Team" writes to `department`, "Role" writes to `job_title`. Start date is a native date picker (`<input type="date">`), so it always submits `YYYY-MM-DD`. The Clinical Apps checkbox label spells out its contents ("Med Dispensing, Revenue Cycle").
